...
class DDPG(object):
    ...
    def _build_a(self, s, scope, trainable):
        with tf.variable_scope(scope):
            s_bn = tf.layers.batch_normalization(s)
            net = tf.layers.dense(s_bn, 100, activation=tf.nn.relu, name='l1', trainable=trainable)
    ...
    def _build_c(self, s, a, scope, trainable):
        with tf.variable_scope(scope):
            ...
            w1_s = tf.get_variable('w1_s', [self.s_dim, 100], trainable=trainable)
            w1_a = tf.get_variable('w1_a', [self.a_dim, 100], trainable=trainable)
            b1 = tf.get_variable('b1', [1, 100], trainable=trainable)
            net = tf.nn.relu(tf.matmul(s_bn, w1_s) + tf.matmul(a_bn, w1_a) + b1)
            net = tf.layers.batch_normalization(net)
